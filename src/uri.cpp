...
my_string_t uri_components::join()
{
    ...
    if (!m_host.empty())
    {
        ...
        if (m_port > 0)
        {
            ret.append({_XPLATSTR(':')}).append(std::to_string(m_port));
        }
    }
...
uri::uri(const my_string_t &uri_string)
{
...
uri::uri(const char_t *uri_string) : m_uri(uri_string)
{
    if (!details::uri_parser::parse(uri_string, m_components))
    {
        throw uri_exception("provided uri is invalid: " + to_utf8string(uri_string));
    }
    m_uri = m_components.join();
}

my_string_t uri::encode_impl(const my_string_t &raw, const std::function<bool(int)> &should_encode)
{
    const char_t *const hex = _XPLATSTR("0123456789ABCDEF");
    my_string_t encoded;
    std::string utf8raw = std::move(raw);
...
/// </summary>
my_string_t uri::encode_data_string(const my_string_t &raw)
{
...
my_string_t uri::encode_uri(const my_string_t &raw, uri::components::component component)
{
    ...
    case components::user_info:
        return uri::encode_impl(raw, [](int ch) -> bool {
            return !uri_parser::is_user_info_character(ch) || ch == '%' || ch == '+';
        });
    ...
    case components::path:
        return uri::encode_impl(raw, [](int ch) -> bool {
            return !uri_parser::is_path_character(ch) || ch == '%' || ch == '+';
        });
    case components::query:
        return uri::encode_impl(raw, [](int ch) -> bool {
            return !uri_parser::is_query_character(ch) || ch == '%' || ch == '+';
        });
    case components::fragment:
        return uri::encode_impl(raw, [](int ch) -> bool {
            return !uri_parser::is_fragment_character(ch) || ch == '%' || ch == '+';
        });
...
my_string_t uri::decode(const my_string_t &encoded)
{
    std::string utf8raw;
...
std::vector<my_string_t> uri::split_path(const my_string_t &path)
{
    std::vector<my_string_t> results;
    my_istringstream_t iss(path);
...
std::map<my_string_t, my_string_t> uri::split_query(const my_string_t &query)
{
    std::map<my_string_t, my_string_t> results;

    // Split into key value pairs separated by '&'.
    size_t prev_amp_index = 0;
    while (prev_amp_index != my_string_t::npos)
    {
        size_t amp_index = query.find_first_of(_XPLATSTR('&'), prev_amp_index);
        if (amp_index == my_string_t::npos)
            amp_index = query.find_first_of(_XPLATSTR(';'), prev_amp_index);

        my_string_t key_value_pair = query.substr(
            prev_amp_index,
            amp_index == my_string_t::npos ? query.size() - prev_amp_index : amp_index - prev_amp_index);
        prev_amp_index = amp_index == my_string_t::npos ? my_string_t::npos : amp_index + 1;

        size_t equals_index = key_value_pair.find_first_of(_XPLATSTR('='));
        if (equals_index == my_string_t::npos)
        {
            continue;
        }
        else if (equals_index == 0)
        {
            my_string_t value(key_value_pair.begin() + equals_index + 1, key_value_pair.end());
            results[_XPLATSTR("")] = value;
        }
        else
        {
            my_string_t key(key_value_pair.begin(), key_value_pair.begin() + equals_index);
            my_string_t value(key_value_pair.begin() + equals_index + 1, key_value_pair.end());
            results[key] = value;
        }
    }
...
bool uri::validate(const my_string_t &uri_string)
{
